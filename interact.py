...
def makeCovarPlotReal(text,sender):
    import image_hosting as ih
    import os 
    
    # get filename
    filename = covarPlotWrapper(text,sender)
    ...
    # delete file 
    DBM.deleteFile(path="/covar_plots",filename=filename)
    DBM.uploadFile(path="/covar_plots",filename=filename)
    temo = DBM.getTemporaryUrl(path="/covar_plots",filename=filename)
    os.remove(filename)
    return temo['url']
...
def makeStockMCPlot(text,sender):
    import image_hosting as ih
    import os 
    ...
    # delete file 
    DBM.deleteFile(path="/stock_mc_plots",filename=filename)
